    ...
    <div>
      <div className="w-full  h-fit   p-2 flex-col-center">
        <div className="flex-center p-1  w-full ">
          <img
            className="max-h-24 md:max-h-28 max-w-20 md:max-w-30 md:mx-2  rounded-[20%]"
            src={user?.avatar_url}
            alt=""
          />
          ...
          <div className="text-[15px]  flex-col items-center shadow-black shadow p-3  m-2 md:max-w-[50%]">
            ...
            <div className="text-[12px]  font-bold font-sans capitalize w-fit ">
              {user?.bio}
            </div>
          </div>
        </div>
        ...
        if (user[dev[0]] && user[dev[0]] !== "")
          return (
            <div
              // @ts-ignore
              key={index + dev[0]}
              className="text-[12px] font-bold capitalize border border-slate-800 rounded mr-[3px] px-1 flex-center"
            >
              {/* @ts-ignore */}
              {dev[1]} {user[dev[0]]}
            </div>
